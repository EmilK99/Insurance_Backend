...
func (a AeroAPI) AirportDelaysRate(f *FlightInfo) (float32, error) {
	...
	if len(apd.FlightCancellationStatisticsResult.Matching) == 0{
		log.Errorf("Can't get airport delays: don't have airport info")
		return 0, errors.New("don't have airport info")
	}
...
func (a AeroAPI) AirlineDelaysRate(f *FlightInfo) (float32, error) {
	...
	if err != nil {
		log.Errorf("Can't get airline delays: %v", err)
		return 0, err
	}
	if len(apd.FlightCancellationStatisticsResult.Matching) == 0{
		log.Errorf("Can't get airline delays: don't have airline info")
		return 0, errors.New("don't have airline info")
	}
	numDelays := apd.FlightCancellationStatisticsResult.Matching[0].Delays

	numDep := apd.FlightCancellationStatisticsResult.Matching[0].Total

	airlineDelayRate := float32(numDelays) / float32(numDep)

	score := airlineDelayRate * 3 * 100
